...
export class JettonWalletContract implements Contract {
  ...
  async getData() {
    const { stack, exit_code } = await this.client.callGetMethodWithError(this.address, 'get_wallet_data', []);

    if (exit_code === -13) return { balance: new BN(0, 10) } // not deployed
    if (exit_code !== 0) {
  ...
  async getTransactions(limit: number = 5) {
